...
#        USAGE:  ./install.sh 
#
#     SYNOPSIS:  Install the Vim plugin bash-support.vim from the current directory
#
...
mkdir  -p $target_dir/codesnippets-bash

#-----------------------------------------------------------------------
#  (2) Copy files
#-----------------------------------------------------------------------

cp $source_dir/doc/bashsupport.txt   $target_dir/doc/

if [ -e $target_dir/ftplugin/sh.vim ]
then
  mv $target_dir/ftplugin/sh.vim $target_dir/ftplugin/sh.vim.save
fi

cp  $source_dir/ftplugin/sh.vim          $target_dir/ftplugin/
cp  $source_dir/doc/bashsupport.txt      $target_dir/doc/
cp  $source_dir/wordlists/bash.list      $target_dir/wordlists/
cp  $source_dir/plugin/bash-support.vim  $target_dir/plugin/
...
then
  #-----------------------------------------------------------------------
  #  (4) Append the file  bash.vimrc  to  .vimrc
  #-----------------------------------------------------------------------

  cat $source_dir/rc/bash.vimrc  >> $HOME/.vimrc
